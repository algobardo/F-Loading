...
namespace Mobile.Util
{
    public class StringMeasure
    {
        ...
        static public Size Measure(Graphics gr, string text, Rectangle rect, bool textboxControl)
        {
            Rect bounds = new Rect(new Rectangle(rect.X, rect.Y, rect.Width - 8, rect.Height));
        ...
        static public Size Measure(Control control, string text, Rectangle rect)
        {
            ...
            using (Graphics gr = Graphics.FromHdc(hDC))
            {
                ...
                result = Measure(gr, text, rect, control is TextBox);
